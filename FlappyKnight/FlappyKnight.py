...
def define_highscore_file(filepath, ingame):
    
    with open(filepath, 'r') as file:
        global noob_highscore, experienced_highscore, pro_highscore, harder_noob_highscore, harder_experienced_highscore, harder_pro_highscore, deaths
    ...
    if ingame == True:
        ...
        # Show an information messagebox with an "OK" button
        messagebox.showinfo("File import", "If you selected a valid file, it has been successfully imported!")

        # Destroy the root window after the message is closed
        root.destroy()

define_highscore_file("highscores.txt", False)
...
def ask_volume():
    ...
    uncalcedvolume = open_slider_popup()

    return uncalcedvolume

def import_highscore_file():
    ...
    global filepathimport
    filepathimport = open_file_dialog(root)
    if filepathimport:
        define_highscore_file(filepathimport, True)
    else:
            # Create the main application window (root)
        root = tk.Tk()
        root.withdraw()  # Hide the root window
        ...
        # Show an information messagebox with an "OK" button
        messagebox.showinfo("File import", "You did not choose a file, so nothing got imported.")

        # Destroy the root window after the message is closed
        root.destroy()

def export_highscore_file():
    def copy_to_desktop(file_name, target_directory):
        # Get the desktop path from the environment variable
        ...
        destination_file = os.path.join(desktop, file_name)

        # Copy the file to the desktop
        shutil.copy2(source_file, destination_file)
    copy_to_desktop("highscores.txt", target_directory)
...
def export_yes_no():
    def ask_yes_no_question():
        ...
        answer = messagebox.askquestion("File export", "This will copy your highscores file onto your desktop. If you already have a highscores.txt on your deksktop, it will be overwritten! Do you want to proceed?")

        # Check the answer and print accordingly
        if answer == 'yes':
            export_highscore_file()
        else:
            pass

        # Destroy the root window after the question is answered
        root.destroy()
...
bg_noob = pygame.image.load('bg.png')
...
def ask_difficulty():
    ...
    # Create the messagebox
        result = messagebox.askyesno("Select difficulty", "Do you want the game to get harder?")
...
while run:
    ...
    draw_text("Deaths:", font, white, int(100), 80)
    draw_text(str(deaths), font, white, int(screen_width / 2), 80)
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT  and beforestart == False:
            run = False
    if beforestart == True:
        beforestart = False
    ...
    if keysw[pygame.K_SPACE] and flying == False and game_over == False:
        flying = True
    if keysw[pygame.K_0] and game_over == True:
        ask_difficulty()
    ...
    if keysw[pygame.K_LALT] and flying == False:
        uncalcvolume = ask_volume()
        volume = uncalcvolume / 100
        pygame.mixer.music.set_volume(volume)
        death_sound.set_volume(volume)

    if keysw[pygame.K_RETURN] and flying == False:
        import_highscore_file()
    
    if keysw[pygame.K_RSHIFT] and flying == False:
        export_yes_no()
